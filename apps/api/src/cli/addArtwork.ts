...
import { createPrismaClient } from '../db/prisma.js';import {
  createOssUploader,
  StubOssUploader,
  type OssUploader
} from '../services/ossUploader.js';
import { ossKeyForOriginal } from './ossKeys.js';
import {
  loadManifest,
  type Manifest,
  type ManifestErrorCategory,
  type ManifestMedia,
  type ManifestParseResult
} from './manifest.js';

export type RunCategory =
  | ManifestErrorCategory
  | 'config-missing'
...
const EXIT_CODES: Record<RunCategory, number> = {
...
export async function runAddArtwork(options: RunOptions): Promise<number> {
  ...
  const planned = manifest.media.map<PlannedMedia>((m) => ({
    manifest: m,
  ...
  let removedLinks: Array<{ mediaAssetId: string; role: string }> = [];
  try {
...
async function applyDatabaseChanges(
  prisma: PrismaClient,
  manifest: Manifest
): Promise<Array<{ mediaAssetId: string; role: string }>> {
  const artworkData = {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const manifestPairs = new Set(
      manifest.media.map((m) => `${m.id}::${m.role}`)
    );
    ...
    const stale = existingLinks.filter(
      (link) => !manifestPairs.has(`${link.mediaAssetId}::${link.role}`)
    );
    ...
    const removedSummary: Array<{ mediaAssetId: string; role: string }> = stale.map((s) => ({
...
function writeManifestError(
  stderr: NodeJS.WritableStream,
  result: Exclude<ManifestParseResult, { ok: true }>
): void {
...
function writeUploadFailure(
  ...
  stderr.write(
    '  hint:     fix the cause and re-run; already-uploaded objects will be skipped via HEAD.\n'
  );
...
function writeDryRunPlan(
  ...
  manifestPath: string,
  manifest: Manifest,
  planned: PlannedMedia[]
): void {
  stdout.write(`[add-artwork] DRY-RUN ${manifestPath}\n`);
  stdout.write(
    `  artwork: ${manifest.artwork.id} (will UPSERT, status=${manifest.artwork.status})\n`
  );
  stdout.write(`  media:\n`);
...
function writeSuccess(
  ...
  uploaded: string[],
  skipped: string[],
  removedLinks: Array<{ mediaAssetId: string; role: string }>
): void {
...
function hintFor(category: ManifestErrorCategory): string {
  switch (category) {
...
function parseArgs(argv: string[]): {
  ...
  for (const arg of argv.slice(2)) {
    ...
    else if (manifestPath === null) manifestPath = arg;
  }
...
async function main(): Promise<void> {
  ...
  let uploader: OssUploader | null = null;
  if (!dryRun) {
    if (stubMode) {
      ...
      process.stderr.write(
        '  hint:     set ALIBABA_CLOUD_ACCESS_KEY_ID and ALIBABA_CLOUD_ACCESS_KEY_SECRET, or re-run with --dry-run.\n'
      );
